...
CreditorService.prototype.getTotalInvestTran = function (invest_id, creditor_invest_id, investType, cb) {
    var conn = dc.main.getConn();
    var sql = "SELECT SUM(tran_price) investTranMoney FROM creditor_loan WHERE invest_id = " + invest_id + " AND status =  0";
    var sql2 = "SELECT SUM(tran_price) investTranMoney FROM creditor_loan WHERE creditor_invest_id = " + creditor_invest_id + " AND status =  0";
    if (investType == 0) {
        conn.execute(sql, [], function (err, data) {
