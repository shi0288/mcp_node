...
var creditorAyfxEngine = engine.creditorAyfxEngine;
 ...
 * @param cb
 */
CreditorService.prototype.getSumCreditor = function (creditor_loan_id, cb){
    var conn = dc.main.getConn();
    //var sql = "SELECT SUM(creditor_value) sum FROM creditor_invest WHERE creditor_loan_id = " + creditor_loan_id + "";
    var sql = "SELECT SUM(creditor_value) sum FROM creditor_invest WHERE status = 1 and creditor_loan_id = " + creditor_loan_id + "";
    conn.execute(sql, [], function (err, data) {
        if (err) {
            console.log(err);
        }else {
 ...
 * @param loan_id
 */
CreditorService.prototype.getAyfxRepayDay = function (loan_id, cb) {
    var conn = dc.main.getConn();
    var sql = "SELECT * FROM loan_repay WHERE loan_id = " + loan_id + " ORDER BY period desc LIMIT 1";
    conn.execute(sql, [], function (err, data) {
        if (err) {
            console.log(err);
            return null;
        }else {
            cb(err, data[0].repay_day);
        }
    });
};
/**
 ...
 * 查询投资记录的已成功转让的金额
 */
CreditorService.prototype.getTotalInvestTran = function (invest_id, creditor_invest_id, investType, cb) {
    var conn = dc.main.getConn();
    var sql = "SELECT SUM(tran_price) investTranMoney FROM creditor_loan WHERE invest_id = " + invest_id + " AND status = 0 OR status = 1";
    var sql2 = "SELECT SUM(tran_price) investTranMoney FROM creditor_loan WHERE creditor_invest_id = " + creditor_invest_id + " AND status = 0 OR status = 1";
    if (investType == 0) {
        conn.execute(sql, [], function (err, data) {
            cb(err, data[0].investTranMoney);
        });
...
/**
 * 查看同一时间的债权转让记录
 * @param invest_id  ---  标的投资id
 */
CreditorService.prototype.getTotalInvestTotal = function (invest_id, cb) {
    var conn = dc.main.getConn();
    var sql = "SELECT COUNT(0) investTotal FROM creditor_loan WHERE invest_id = " + invest_id + " OR creditor_invest_id = " + invest_id + " AND status =  0";
    conn.execute(sql, [], function (err, data) {
       if (err) {
           console.log(err);
           cb(err, null);
       }else {
           cb(err, data[0].investTotal);
       }
    });
};
/**
 ...
 * @param cb
 */
CreditorService.prototype.buyTran = function (money, userID, accept_money,status,create_time,creditor_loan_id, loanRepayObj, creditorLoanObj,investObj,loanObj,curRepayDay,curDay,preRepayDay,creditorValueInvest,cre_inv_suc_man,surMoney, cb) {
    ...
    var conn = dc.main.getConn();
    var sql = "UPDATE account SET money = money" + "-" + accept_money  + " WHERE user_id = '" + userID + "' ";
    ...
    var sql6 = "UPDATE creditor_invest SET status = 0 WHERE id = '" + creditor_invest_id + "' ";
    var sql7 = "";
    conn.execute(sql, [], function (err, data) {
        if(err) {
            console.log(err);
                                        ...
                                        if(err) {
                                            ...
                                            switch (creditorLoanObj.creditor_type) {
                                                case 'debx' :
                                                    ...
                                                    break;
                                                case 'ayfx':
                                                    creditorAyfxEngine.ayfxCreditor(id,money,creditorLoanObj,i,j,loanObj,loanRepayObj,curRepayDay,curDay,preRepayDay,data,investObj,surMoney,function (err) {
                                                        if (err) {
                                                            console.log(err);
                                                        }
                                                    });
