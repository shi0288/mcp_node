...
CreditorControl.prototype.handleCR06 = function (headNode, bodyNode, cb) {
    ...
    var repay_daycur;var loan_id;var debxCount;var sur_period_debx;var interest;
    var preRepayDay;var period;var accrue_time;var ycxhAccurDate;
    var sqll;var sqll2;var sqll3;
    //结果集
    var creditorBodyNode = {};var arr = new Array();
    //可转让债权, 可转让总额, 转让中的债权, 债权价值总额, 已转让的债权, 已转让总额, 已认购的债权, 折让金总额
    var sql1 = "SELECT COUNT(0) FROM invest WHERE user_id = '" + userID + "' AND status =0";
    var sql2 = "SELECT COUNT(0) FROM creditor_invest WHERE user_id = '" + userID + "' AND status =0";
    //申请转让的列表
    var sql3 = "SELECT i.id, l.name, l.rate, (to_days(rr.repay_day) - to_days(now())) days, tt.repay_daycur, i.creditor_value_update, " +
        "i.money, i.create_time, l.repay_type, l.id loan_id, tt.period, l.accrue_time, tt.interest " +
        "FROM invest i LEFT JOIN loan l ON l.id = i.loan_id LEFT JOIN (SELECT (z.repay_day)repay_day, (z.loan_id)loan_id FROM " +
        "(SELECT * FROM loan_repay ORDER BY period DESC) z GROUP BY z.loan_id ORDER BY z.period DESC) rr ON i.loan_id = rr.loan_id " +
    ...
    var sql5 = "";
    var sql4 = "SELECT l.id, l.name, l.rate, (to_days(rr.repay_day) - to_days(now())) days, tt.repay_daycur, ci.creditor_value_update, ci.money " +
        "FROM creditor_invest ci LEFT JOIN loan l ON l.id = ci.loan_id LEFT JOIN (SELECT (z.repay_day)repay_day, (z.loan_id)loan_id " +
        "FROM (SELECT * FROM loan_repay ORDER BY period DESC) z GROUP BY z.loan_id ORDER BY z.period DESC) rr ON l.id = rr.loan_id " +
        "LEFT JOIN (SELECT (t.repay_day)repay_daycur,(t.loan_id)loan_id FROM loan_repay t WHERE t.status = 0 GROUP BY t.loan_id ORDER BY period) tt " +
        "ON rr.loan_id = tt.loan_id";
            ...
            conn.execute(sql3, [], function (err, data) {
                ...
                if (err) {
                    ...
                    async.eachSeries(data, function (investObj, cb) {
                        ...
                        console.log("************************");
                        console.log(id + "," + invest_time + "," + repay_type + "," + loan_id + "," + days + "," + period);
                        //检验数据
