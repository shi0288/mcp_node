...
CreditorControl.prototype.handleCR01 = function (headNode, bodyNode, cb) {
    var creditorTranBodyNode = {}; var arr = [];var obj = {};
    async.waterfall([
        //验证债权转让的条件
        function (cb) {
            var creType = 0;
                    ...
                    if (err) {
                        ...
                        creditorService.getMoreLastCreditor(creditor_investObj.id, function (err, data) {
                            if (err) {
                                ...
                                if(data != null) {
                                    var lastCreditorDay = data.termi_time.Format("yyyy-MM-dd");
                        ...
                        creditorService.getLastCreditor(investObj.id, function (err, data) {
                            if (err) {
                                ...
                                if(data != null) {
                                    ...
                                    var lastCreditorDay = moment(data.termi_time).format('YYYY-MM-DD');
            ...
            repay_typeTab.findOne({id: loanObj.repay_type}, {}, [], function (err, repay_typeObj) {
                switch (repay_typeObj.type) {
                    ...
                    case 'debx':
                        ...
                        //creditorTranBodyNode.rst.creditorValue = creditorValue;
                    ...
                    case 'ycxh':
                        ...
                        var ycxhAccurDate = moment(loanObj.accrue_time).format('YYYY-MM-DD');
        ...
        function (id, cb) {
            ...
            var repay_money = bodyNode.creditor_value - bodyNode.disc_price;
            console.log("本次债权的预回款金额为：" + repay_money);
            console.log("本次债权的转让金额为：" + bodyNode.total_price);
            obj['creditor_value'] = bodyNode.creditor_value;
            obj["total_price"] = bodyNode.total_price;
            obj["disc_rate"] = bodyNode.disc_rate;
            obj["repay_money"] = repay_money;
            arr.push(obj);
            creditorTranBodyNode.rst = arr;
            var loan = dc.main.get('creditor_loan');
            loan.save(bodyNode, [], function (err) {
                if (err) {
                    cb(ec.E9050);
                } else {
                    cb(null, creditorTranBodyNode);
                }
            });
        }
    ], function (err, creditorTranBodyNode) {
        cb(err, creditorTranBodyNode);
    });
};
...
CreditorControl.prototype.handleCR06 = function (headNode, bodyNode, cb) {
    ...
    var buy_loan_days =0;var nums;var interva;
    ...
    var tran_apply_cur_repay;var loanInterest;var tran_priceMonInter;
    var sqll;var sqll2;var sqll3;
    //结果集
    var creditorBodyNode = {};var arr = new Array();
    var userTranMoney=0;var userCreditorValue=0;
                            ...
                            async.eachSeries(data, function (investObj, cb) {
                                ...
                                if (curDay2 > repay_daycur2) {
                                    ...
                                    if (gap < tran_apply_cur_repay) {
                                        ...
                                        //检验数据
                                        var sql5 = "SELECT * FROM creditor_loan WHERE ( invest_id = '" + id + "' OR creditor_invest_id = '" + id +"' ) AND status = 1 ORDER BY termi_time DESC LIMIT 1";
                                        var sql6 = "SELECT COUNT(0) inTranCount FROM creditor_loan WHERE ( invest_id = '" + id + "' OR creditor_invest_id = '" + id +"' ) AND status = 0";
                                        ...
                                        //var sql9 = "SELECT COUNT(0) hasTranCount FROM creditor_loan WHERE invest_id = '" + id + "' OR creditor_invest_id = '" + id +"' AND status = 1";
                                        conn.execute(sql6, [], function (err, data) {
                                            var count;
                                            if (err) {
                                                console.log(err);
                                            }else {
                                                console.log(sql6);
                                                count = data[0].inTranCount;
                                                console.log("原标投资编号" + id + "正在转让的债权数量为：" + count);
                                                if (count == 1) {
                                                    console.log("原标投资编号" + id + "债权转让进行中");
                                                    delete investObj;
                                                    investCount--;
                                                    cb(err);
                                                }else if (count > 1){
                                                    console.log("原标投资编号" + id + "债权转让同一记录有多条转让记录，请人工仔细审核");
                                                    delete investObj;
                                                    investCount--;
                                                    cb(err);
                                                }else {
                                                    var x = false;var preCreditorDay;
                                                    conn.execute(sql5, [], function (err, data) {
                                                        if (err) {
                                                            ...
                                                            switch (x) {
                                                                case 1 :
                                                                    var termi_time = moment(data[0].termi_time).format('YYYY-MM-DD');
                                                                    //console.log("-------**********----------" + termi_time);
                                                                    preCreditorDay = dateMathUtil.daysBetween(curDay, termi_time);
                                                                    console.log("距上次完成间隔天数为：" + preCreditorDay + "," + interva);
                                                                    if (preCreditorDay <= interva) {
                                                                        delete investObj;
                                                                        investCount--;
                                                                        cb(err);
                                                                        break;
                                                                        ...
                                                                        if (err) {
                                                                            ...
                                                                            conn.execute(sql8, [], function (err, data) {
                                                                                if (err) {
                                                                                    ...
                                                                                    //转让成功的代收本息
                                                                                    tran_priceMonInter = data[0].tran_price/investObj.money*loanInterest + data[0].tran_price;
                                                                                    console.log(data[0].tran_price + "待收本息为：" + tran_priceMonInter);
                                                                                                            ...
                                                                                                            if (data[0].type == "debx") {
                                                                                                                ...
                                                                                                                conn.execute(sqll2, [], function (err, data) {
                                                                                                                    if (err) {
                                                                                                                        ...
                                                                                                                        if (debxCount < sur_period_debx) {
                                                                                                                            ...
                                                                                                                            investObj.tranMoney = tranMoney;
                                                                                                                            investObj.repay_daycur = moment(investObj.repay_daycur).format('YYYY-MM-DD');
                                                                                                                            userTranMoney += tranMoney;
                                                                                                                            userCreditorValue += creditorDebxValue;
                                                                                                                            arr.push(investObj);
                                                                                                                            cb(err);
                                                                                                                        }
                                                                                                                    }
                                                                                                                ...
                                                                                                                if (days < sur_period_other) {
                                                                                                                    ...
                                                                                                                    if (data[0].type == "ayfx") {
                                                                                                                        ...
                                                                                                                        investObj.repay_daycur = moment(investObj.repay_daycur).format('YYYY-MM-DD');
                                                                                                                        userTranMoney += tranMoney;
                                                                                                                        userCreditorValue += creditorAyfxValue;
                                                                                                                        arr.push(investObj);
                                                                                                                        cb(err);
                                                                                                                        ...
                                                                                                                        investObj.repay_daycur = moment(investObj.repay_daycur).format('YYYY-MM-DD');
                                                                                                                        userTranMoney += tranMoney;
                                                                                                                        userCreditorValue += creditorYcxhValue;
                                                                                                                        arr.push(investObj);
                                                                                                                        cb(err);
                                                                                                                    }
                                                                                                                }
                                ...
                                creditorBodyNode.userTranMoney = userTranMoney;
                                creditorBodyNode.userCreditorValue = userCreditorValue;
                                cb(null, creditorBodyNode);
                            });
                        }
                    }, {dateToString: true});
...
/**
 * @Description  CR07 个人中心的债权转让详情
 * @param headNode
 * @param bodyNode
 * @param cb
 */
CreditorControl.prototype.checkCR07 = function (headNode, bodyNode, cb) {
    cb(null);
};

CreditorControl.prototype.handleCR07 = function (headNode, bodyNode, cb) {
    var userDetailCount = 0;
    var userID = bodyNode.cond.userID;
    var investID = bodyNode.cond.investID;
    //债权规则效验的条件
    var curDay = moment(new Date()).format("YYYY-MM-DD");
    var curDay2 = moment(new Date()).format("YYYY-MM-DD HH:mm");
    var buy_loan_days =0;
    ...
    //结果集
    var creditorBodyNode = {};var arr = new Array();
    var sql1 = "SELECT COUNT(0) userDetailCount " +
        "FROM (SELECT i.id, i.loan_id, i.money, i.interest, i.create_time FROM invest i UNION ALL SELECT ci.id, ci.loan_id, ci.money, ci.interest, ci.create_time " +
        "FROM creditor_invest ci) li LEFT JOIN loan l ON l.id = li.loan_id LEFT JOIN (SELECT (z.repay_day)repay_day, (z.loan_id)loan_id FROM " +
        "(SELECT * FROM loan_repay ORDER BY period DESC) z GROUP BY z.loan_id ORDER BY z.period DESC) rr ON li.loan_id = rr.loan_id " +
        "LEFT JOIN (SELECT (t.repay_day)repay_daycur,(t.period)period,(t.interest)interest,(t.loan_id)loan_id FROM loan_repay t " +
        "WHERE t.status = 0 GROUP BY t.loan_id ORDER BY period) tt " +
        "ON rr.loan_id = tt.loan_id WHERE li.id = '" + investID + " '";
    //申请转让的列表
    var sql3 = "SELECT li.id, l.name, l.rate, (to_days(rr.repay_day) - to_days(now())) days, tt.repay_daycur, " +
        "li.money, (li.interest) loanInterest, li.create_time, l.repay_type, l.id loan_id, tt.period, l.accrue_time, tt.interest " +
        "FROM (SELECT i.id, i.loan_id, i.money, i.interest, i.create_time FROM invest i UNION ALL SELECT ci.id, ci.loan_id, ci.money, ci.interest, ci.create_time " +
        "FROM creditor_invest ci) li LEFT JOIN loan l ON l.id = li.loan_id LEFT JOIN (SELECT (z.repay_day)repay_day, (z.loan_id)loan_id FROM " +
        "(SELECT * FROM loan_repay ORDER BY period DESC) z GROUP BY z.loan_id ORDER BY z.period DESC) rr ON li.loan_id = rr.loan_id " +
        "LEFT JOIN (SELECT (t.repay_day)repay_daycur,(t.period)period,(t.interest)interest,(t.loan_id)loan_id FROM loan_repay t " +
        ...
        if (err) {
            ...
            conn.execute(sql1, [], function (err, data) {
                if (err) {
                    ...
                    userDetailCount = data[0].userDetailCount;
                    console.log("查询到的投资记录总数为：" + userDetailCount);
                    if (userDetailCount == 1) {
                        conn.execute(sql3, [], function (err, data) {
                            //执行
                            if (err) {
                                console.log(err);
                                ...
                                async.eachSeries(data, function (investObj, cb) {
                                    ...
                                    period = investObj.period;
                                    interest = investObj.interest;
                                    loanInterest = investObj.loanInterest;
                                    accrue_time = investObj.accrue_time;
                                    repay_type = investObj.repay_type;
                                    invest_time = moment(investObj.create_time).format("YYYY-MM-DD");
                                    repay_daycur = moment(investObj.repay_daycur).format("YYYY-MM-DD");
                                                ...
                                                if (err) {
                                                    ...
                                                    conn.execute(sql8, [], function (err, data) {
                                                        if (err) {
                                                            ...
                                                            console.log(data[0].tran_price + "待收本息为：" + tran_priceMonInter);
                                                            investObj.money -=  data[0].tran_price;
                                                            investObj.investMonInter = investObj.money + investObj.interest - tran_priceMonInter;
                                                            console.log(investObj.money + "待收本息为：" + investObj.investMonInter);
                                                            tranMoney = investObj.money;
                                                            if (tranMoney < 0) {
                                                                console.log("原标投资编号" + id + "可转让金额错误，请人工审核");
                                                                cb(err);
                                                                                        ...
                                                                                        conn.execute(sqll2, [], function (err, data) {
                                                                                            if (err) {
                                                                                                ...
                                                                                                if (debxCount < sur_period_debx) {
                                                                                                    ...
                                                                                                    investObj.tranMoney = tranMoney;
                                                                                                    investObj.repay_daycur = moment(investObj.repay_daycur).format('YYYY-MM-DD');
                                                                                                    investObj.middlePart = creditorCalculateService.creditorValueDeMiddle(repay_daycur, curDay, preRepayDay, interest, surMoney);
                                                                                                    arr.push(investObj);
                                                                                                    cb(err);
                                                                                                }
                                                                                            }
