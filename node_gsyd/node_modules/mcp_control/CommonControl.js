...
CommonControl.prototype.handleCM07 = function (headNode, bodyNode, cb) {
    userService.getUserList(bodyNode.cond, bodyNode.skip, bodyNode.limit, function (err, data, count) {
        if (err) {
            cb(ec.E9999);
        } else {
            var backNode = {};
 ...
 * @param bodyNode
 * @param cb
 * @Description  CM10 获取银行开户地区代码信息
 */

CommonControl.prototype.checkCM10 = function (headNode, bodyNode, cb) {
    cb(null);
};

CommonControl.prototype.handleCM10 = function (headNode, bodyNode, cb) {
    var conn = dc.main.getConn();
    var sql = "";
    if (bodyNode.proCode == undefined) {
        sql = "select a.pro_code,a.province from fuiou_city a GROUP BY a.province ORDER BY city_code";
    } else {
        sql = "select a.city_code,a.city from fuiou_city a where a.pro_code='" + bodyNode.proCode + "' ORDER BY city_code";
    }
 ...
 * @param bodyNode
 * @param cb
 * @Description  CM11 获取记录数量
 */

CommonControl.prototype.checkCM11 = function (headNode, bodyNode, cb) {
    //检查参数是否完整
    if (bodyNode.table == null || bodyNode.table == undefined) {
        cb(ec.E3070);
        return;
...
CommonControl.prototype.handleCM11 = function (headNode, bodyNode, cb) {
    var table = dc.main.get(bodyNode.table);
    var cursor = table.find(bodyNode.cond, {}, []).sort(bodyNode.sort).limit(bodyNode.skip, bodyNode.limit);
    var backBodyNode = {};
 ...
 * @param bodyNode
 * @param cb
 * @Description  CM12 获取某字段SUM
 */

CommonControl.prototype.checkCM12 = function (headNode, bodyNode, cb) {
    //检查参数是否完整
    if (bodyNode.table == null || bodyNode.table == undefined) {
        cb(ec.E3070);
        return;
    ...
    async.waterfall([
        ...
        //第三步检查是否有条件字段
        function (checkCb) {
            checkService.checkTableColumn(table, bodyNode.cons, function (err) {
                checkCb(err);
            });
        }
    ], function (err) {
...
CommonControl.prototype.handleCM12 = function (headNode, bodyNode, cb) {
    var table = dc.main.get(bodyNode.table);
    var cursor = table.find(bodyNode.cond, {}, []).sort(bodyNode.sort).limit(bodyNode.skip, bodyNode.limit);
    var backBodyNode = {};
    cursor.sum(bodyNode.cons,function (err, count) {
        backBodyNode.sum = count;
        cb(null, backBodyNode);
    });
};
