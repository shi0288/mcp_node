...
AccountControl.prototype.handleAC06 = function (headNode, bodyNode, cb) {
    ...
    var loanStr = '';
    if (bodyNode.loanId != undefined && bodyNode.loanId != '') {
        loanStr = " and lo.id='" + bodyNode.loanId + "'";
    }
    var statusStr = "";
    if (bodyNode.status != undefined && bodyNode.status == 1) {
        statusStr = " lr.status=1";
    } else if (bodyNode.status != undefined && bodyNode.status == 2) {
        statusStr = " 1=1";
    } else {
        statusStr = " lr.status=0";
    }
    var sql = "select sum(lr.money) as money,sum(lr.interest+lr.st_interest) as interest from loan_repay lr LEFT JOIN loan lo on lr.loan_id=lo.id where " + statusStr +
...
AccountControl.prototype.handleAC07 = function (headNode, bodyNode, cb) {
    ...
    var sql = "select sum(a.money) as money from invest a LEFT JOIN loan b on a.loan_id=b.id  " +
        " where a.status=1200 and (b.status=1200 or b.status=1300) and a.user_id='" + bodyNode.userId + "' " + typeStr;
    conn.execute(sql, [], function (err, data) {
        if (err) {
            cb(ec.E9999);
        } else {
...
AccountControl.prototype.handleAC08 = function (headNode, bodyNode, cb) {
    ...
    var statusStr = '';
    if (bodyNode.status != undefined && bodyNode.status != '') {
        statusStr = " and b.status='" + bodyNode.status + "'";
    }
    var sql = "select count(*) as countNum from invest a LEFT JOIN loan b on a.loan_id=b.id  " +
        " where a.status='1200' and  a.user_id='" + bodyNode.userId + "' " + typeStr + statusStr;
    conn.execute(sql, [], function (err, data) {
        if (err) {
            cb(ec.E9999);
        } else {
 ...
 * @param bodyNode
 * @param cb
 * @Description  AC11 查询借款计划中待回款的本金，利息，罚息
 */
AccountControl.prototype.checkAC11 = function (headNode, bodyNode, cb) {
    cb(null);
};

AccountControl.prototype.handleAC11 = function (headNode, bodyNode, cb) {
    var conn = dc.main.getConn();
    var backNode = {};
    var entity = {};
    backNode.entity = entity;
    var sql = "SELECT count(lr.id) as nums,sum(lr.money) as money,sum(lr.interest+lr.st_interest) as interest,sum(lr.fee+lr.penalty+lr.advance_penalty) as penalty " +
        "from loan_repay lr LEFT JOIN loan lo on lr.loan_id=lo.id where lr.status=0 ";
    var condition = " and lo.repay_user_id='" + bodyNode.userId + "'";
    sql += condition;
    conn.execute(sql, [], function (err, data) {
        if (err) {
            cb(ec.E9999);
 ...
 * @param bodyNode
 * @param cb
 * @Description  AC12 查询已还的罚息
 */

AccountControl.prototype.checkAC12 = function (headNode, bodyNode, cb) {
    if (bodyNode.userId == undefined || bodyNode.userId == '') {
        cb(ec.E9999);
    } else {
        cb(null);
...
AccountControl.prototype.handleAC12 = function (headNode, bodyNode, cb) {
    var conn = dc.main.getConn();
    var backNode = {};
    var entity = {};
    backNode.entity = entity;
    var sql = "select sum(lr.money) as money,sum(lr.interest+lr.st_interest) as interest,sum(lr.fee+lr.penalty+lr.advance_penalty) as penalty from loan_repay lr LEFT JOIN loan lo on lr.loan_id=lo.id where " +
        " lo.repay_user_id='" + bodyNode.userId + "'";
    conn.execute(sql, [], function (err, data) {
        if (err) {
            cb(ec.E9999);
        } else {
 ...
 * @param bodyNode
 * @param cb
 * @Description  AC13 获取红包信息
 */

AccountControl.prototype.checkAC13 = function (headNode, bodyNode, cb) {
    //检查参数是否完整
    if (bodyNode.user_id == null || bodyNode.user_id == undefined || bodyNode.user_id == '') {
        cb(ec.E3070);
        return;
    }
    cb(null);
};
AccountControl.prototype.handleAC13 = function (headNode, bodyNode, cb) {
    var sql = "SELECT a.id,a.coupon_id,a.create_time,a.status,b.name,b.type,b.user_cond,b.money,b.start_time,b.end_time,b.description " +
        "from my_coupon a LEFT JOIN coupon b on a.coupon_id=b.id " +
        "where a.user_id='" + bodyNode.user_id + "'";
    ...
    if (bodyNode.status != null && bodyNode.status != undefined) {
        condiStr += " and a.status='" + bodyNode.status + "'";
    }

    if (bodyNode.start != null && bodyNode.start != undefined && bodyNode.start != '') {
        condiStr += " and b.start_time<='" + bodyNode.start + "'";
        condiStr += " and b.end_time>'" + bodyNode.start + "'";
    }
    ...
    conn.execute(sql, [], function (err, data) {
        if (err) {
            ...
            for (var key in data) {
                ...
                data[key]['start_time'] = moment(data[key]['start_time']).format("YYYY-MM-DD");
                data[key]['end_time'] = moment(data[key]['end_time']).format("YYYY-MM-DD");
            }
            backBodyNode.rst = data;
