        ...
        if (refreeData[0].refreeNum && refreeData[0].refreeNum > 0) {
            ...
            conn.execute(loanerNumSql, [], function (err, loanerData) {
                if (loanerData[0].loanerNum && loanerData[0].loanerNum > 0) {
                    ...
                    var loanerMoneySql = "select sum(inv.money) as loanerMoney from invest inv LEFT JOIN loan lo on inv.loan_id=lo.id where inv.user_id in(select id from user where referrer='" + bodyNode.username + "') and inv.status=1200 and lo.status>=1400  GROUP BY inv.user_id";
