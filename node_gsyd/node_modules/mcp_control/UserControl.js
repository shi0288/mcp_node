...
UserControl.prototype.handleUC05 = function (headNode, bodyNode, cb) {
    ...
    conn.execute(refreeNumSql, [], function (err, refreeData) {
        ...
        if (refreeData[0].refreeNum && refreeData[0].refreeNum > 0) {
            ...
            //如果有被推荐人，继续查询是否有投资
            var loanerNumSql = "select inv.user_id as loanerNum from invest inv LEFT JOIN loan lo on inv.loan_id=lo.id where inv.user_id in(select id from user where referrer='" + bodyNode.username + "') and inv.status=1200 and lo.status>=1400  GROUP BY inv.user_id";
            ...
            conn.execute(loanerNumSql, [], function (err, loanerData) {
                if (loanerData[0].loanerNum && loanerData[0].loanerNum > 0) {
                    //如果有借款人，继续查询投资金额
                    console.log("loanerNumSql:"+loanerData.length);
                    console.log("loanerNumSql:"+loanerData.size);
                    console.log("loanerNumSql:"+loanerData.length());
                    console.log("loanerNumSql:"+loanerData.size());
                    backNode.entity.loanerNum = loanerData.length;
