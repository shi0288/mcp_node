...
UserControl.prototype.handleUC05 = function (headNode, bodyNode, cb) {
    ...
    var backNode = {};
    console.log("refreeNumSql:"+refreeNumSql);
    conn.execute(refreeNumSql, [], function (err, refreeData) {
        ...
        if (refreeData[0].refreeNum && refreeData[0].refreeNum > 0) {
            ...
            //如果有被推荐人，继续查询是否有投资
            var loanerNumSql = "select count(inv.user_id) as loanerNum from invest inv LEFT JOIN loan lo on inv.loan_id=lo.id where inv.user_id in(select id from user where referrer='" + bodyNode.username + "') and inv.status=1200 and lo.status>=1400  GROUP BY inv.user_id";
            console.log("loanerNumSql:"+loanerNumSql);
            conn.execute(loanerNumSql, [], function (err, loanerData) {
                if (loanerData[0].loanerNum && loanerData[0].loanerNum > 0) {
                    ...
                    var loanerMoneySql = "select count(inv.money) as loanerMoney from invest inv LEFT JOIN loan lo on inv.loan_id=lo.id where inv.user_id in(select id from user where referrer='" + bodyNode.username + "') and inv.status=1200 and lo.status>=1400  GROUP BY inv.user_id";
                    console.log("loanerMoneySql:"+loanerMoneySql);
