                    ...
                    async.eachSeries(data, function (set, callback) {
                        ...
                        userBillService.frozenOutMoney(set.user_id,
                            ...
                            userBillCons.payType.GIVE_MONEY_TO_BORROWER,
                                ...
                                if (!err) {
                                    actualMoney += set.money;
                                    if (set.coupon_money != undefined) {
                                        userBillService.thawCouponMoney(set.user_id,
                                            set.coupon_money,
                                            userBillCons.payType.USE_COUPON,
                                                "红包使用：取出代金红包。借款ID:" + loan.id,
                                            function (err, data) {
                                                callback(null);
                                            });
                                    } else {
                    ...
                    if (err) {
                        ...
                        cursor.toArray(function (err, data) {
                            async.eachSeries(data, function (investObj, callback) {
                                if (investObj.coupon_money) {
                                    investObj.money = investObj.money - investObj.coupon_money;
                                }
                                ...
                                userBillService.thawMoney(investObj.user_id,
                                    ...
                                    des, function (err) {
                                        if(investObj.coupon_money){
                                            userBillService.thawCouponMoney(investObj.user_id,
                                                investObj.coupon_money,
                                                userBillCons.payType.USE_COUPON,
                                                    "出借：" + loan.name + "流标,解冻红包金额:"+investObj.coupon_money/100+"。" ,
                                                function (err1) {
                                                    callback(err1);
                                                });
                                        }else{
...
LoanControl.prototype.handleLC07 = function (headNode, bodyNode, cb) {
    var table = dc.main.get(bodyNode.table);
    var sql = "select a.*,b.realname as repayRealname,b.username as repayName,e.realname  as borrowRealname,e.username  as borrowName,c.name as repayTypeName,c.type as code,c.unit,d.name as loanTypeName " +
    ...
    conn.execute(sql, [], function (err, data) {
        if (err) {
            ...
            for (var key in data) {
                ...
                if (data[key]['final_time'] != undefined) {
                    data[key]['final_time'] = moment(data[key]['final_time']).format("YYYY-MM-DD HH:mm:ss");
                }
                if (data[key]['accrue_time'] != undefined) {
                    data[key]['accrue_time'] = moment(data[key]['accrue_time']).format("YYYY-MM-DD HH:mm:ss");
                }
            }
...
LoanControl.prototype.handleLC11 = function (headNode, bodyNode, cb) {
    ...
    var sql = "SELECT b.id,b.name,b.accrue_time,rt.name as repayTypeName,dic.name as loanTypeName,b.agreement," +
        ...
        "LEFT JOIN loan b on a.loan_id=b.id LEFT JOIN loan_repay lre on b.id= lre.loan_id and ire.period=lre.period " +
        "LEFT JOIN repay_type rt on rt.id=b.repay_type " +
        "LEFT JOIN dict dic on dic.id=b.loan_type " +
        "LEFT JOIN user repayUser on b.repay_user_id=repayUser.id " +
        "LEFT JOIN user borrowUser on borrowUser.id=b.borrow_user_id " +
