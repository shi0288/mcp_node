...
InvestControl.prototype.handleIC07 = function (headNode, bodyNode, cb) {
    var conn = dc.main.getConn();
    var sql = "SELECT ";
    var condition = "loan.name,invest.id,invest.create_time,invest.money,loan.rate,invest.money,loan.deadline,loan.status";
    var whCondition = " from invest LEFT JOIN loan  on invest.loan_id=loan.id where invest.status=1200  and loan.status>=1200 and  invest.user_id='" + bodyNode.cond.id + "'";
    var statusStr = "";
    ...
    var exSql = sql + condition + whCondition + statusStr + loanTypeStr;
    exSql += '  order by invest.create_time desc';
    exSql += '  limit ' + bodyNode.skip;
    exSql += ',' + bodyNode.limit;
    conn.execute(exSql, [], function (err, data) {
...
InvestControl.prototype.handleIC08 = function (headNode, bodyNode, cb) {
    var conn = dc.main.getConn();
    var sql = "select inre.*,lo.name,lo.status as loanStatus from invest_repay inre LEFT JOIN invest inv on inre.invest_id=inv.id LEFT JOIN loan lo on inv.loan_id=lo.id where 1 = 1 ";
...
InvestControl.prototype.handleIC09 = function (headNode, bodyNode, cb) {
    var conn = dc.main.getConn();
    var sql = "SELECT lr.*,lo.name from loan_repay lr LEFT JOIN loan lo on lr.loan_id=lo.id where 1=1 ";
