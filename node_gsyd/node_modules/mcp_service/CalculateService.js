...
CalculateService.prototype.outRhjfAyfx = function (loanObj, moneyTemp) {
    ...
    finalPeriod.repay_day = moment(finalTime).format("YYYY-MM-DD HH:mm:ss");
    ...
    for (var i = monthNum; i > 0; i--) {
        ...
        if (monthNum == 1) {
            var firstPeriodObj = {};
            firstPeriodObj.repay_day = moment(accrueTime).format("YYYY-MM-DD HH:mm:ss");
            firstPeriodObj.money = 0;
            var tempAccTimeNext = dateMathUtil.dateMonthAdd(finalTime, day_month, m - 1);
            var dayNum = dateMathUtil.dateDiff('d', finalTime, accrueTime);
            var dayNumOut = dateMathUtil.dateDiff('d', finalTime, tempAccTimeNext);
            firstPeriodObj.interest = Math.round(dayNum / dayNumOut * everyMonthInterest);
            if(firstPeriodObj.interest>0){
        ...
        if (m == 0) {
            ...
            if (dateMathUtil.dateDiff('d', temp, finalTime) < 0) {
                ...
                finalChildPeriod.repay_day = moment(temp).format("YYYY-MM-DD HH:mm:ss");;
...
//                console.log(moment(tempAccTime).format("YYYY-MM-DD"));
            ...
            if (dateMathUtil.dateDiff('d', accrueTime, tempAccTime) > 0) {
                ...
                if (tempAccDay <= realRepayDay) {
                    //正常第一期
                    var periodObj = {};
                    periodObj.repay_day = moment(temp).format("YYYY-MM-DD HH:mm:ss");;
                    periodObj.money = 0;
                ...
                var preAccTime = dateMathUtil.dateMonthAdd(finalTime, day_month, m + 1);
                if(dateMathUtil.dateDiff('d', temp, accrueTime) > 0){
                    var dayNum = dateMathUtil.dateDiff('d', preAccTime, accrueTime);
                    var dayNumOut = dateMathUtil.dateDiff('d', preAccTime, tempAccTime);
                    var periodObj = {};
                    periodObj.repay_day = moment(temp).format("YYYY-MM-DD HH:mm:ss");;
                ...
                if(dateMathUtil.dateDiff('d', temp, tempAccTime) >= 0){
                    //本次为整月
                    var periodObj = {};
                    periodObj.repay_day = moment(temp).format("YYYY-MM-DD HH:mm:ss");
                    periodObj.money = 0;
