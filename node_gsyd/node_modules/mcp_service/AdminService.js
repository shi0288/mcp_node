...
AdminService.prototype.userLoginByMobile = function (mobile, password, cb) {
    var conn = dc.main.getConn();
    var tempPass = digestUtil.sha1(password);
    var tag = false;
    var sql = "SELECT * FROM user WHERE mobile='" + mobile + "'  and password='" + tempPass + "' and permission=0 and status=1";
    conn.execute(sql, [], function (err, data) {
        if (err) {
            cb(err, null);
        } else {
...
AdminService.prototype.marketLogin = function (username, password, cb) {
    var conn = dc.main.getConn();
    var tempPass = digestUtil.sha1(password);
    var tag = false;
    var sql = "SELECT * FROM user WHERE username='" + username + "'  and password='" + tempPass + "' and status=1";
    conn.execute(sql, [], function (err, data) {
        if (err) {
            cb(err, null);
        } else {
