 ...
 * 债权购买
 */
CreditorDebxEngine.prototype.debxCreditor = function (id,money,creditorLoanObj,i,j,loanObj,loanRepayObj,curRepayDay,curDay,preRepayDay,data,investObj,surMoney, cb) {
    var k = i;
    //业务参数处理
    var money2 = 0.00;
    var interest2 = 0.00;
    ...
    //受让金额的总利息
    var totalInterest = Math.round((money/(loanObj.money))*creditorData);
                    ...
                    creditor_invest_repayTab.save(cond, [], function (err) {
                        if(err) {
                            ...
                            investTab.findOne({id: creditorLoanObj.invest_id}, {}, [], function (err, investObj) {
                                if (investObj != null) {
                                    invest_repayTab.update({id: investRepayId}, {$inc: { tran_money : money2, tran_interest : interest2 }}, [], function (err) {
                                        if (err) {
                                            console.log(err);
                                        }else {
                                            i++;
                                    ...
                                    creditor_invest_repayTab.update({id: investRepayId}, {$inc: { tran_money : money2, tran_interest : interest2 }}, [], function (err) {
                                        if (err) {
                                            console.log(err);
                                        }else {
                                            i++;
