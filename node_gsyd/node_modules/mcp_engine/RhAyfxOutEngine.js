 ...
 * @param cb
 */
RhAyfxOutEngine.prototype.pt = function (loanObj, repayTypeObj, jt, cb) {
                        ...
                        async.eachSeries(loanRepayObjArr, function (obj, repayCall) {
                            ...
                            if (loanRepayObjArr[_index] == undefined) {
                                ...
                                if (period == 1) {
                                    ...
                                    var repayDay;
                                    if(loanObj.rhday>=tempDay){
                                        repayDay=dateMathUtil.pointDay(tempMonth,loanObj.rhday);
                                    }else{
                                        repayDay=dateMathUtil.pointDay(tempMonth+1,loanObj.rhday);
                                    }
                                    loanRepayObjArr[_index].repay_day = repayDay;
                                    //日利息=[（第一次返息日-线上起息日）/（第一次返息日-线下起息日）] * 投资金额 * 年利率 / 12
                                    //计息日到第一次还款日时间差
                                    var realFirstAccrueDay=dateMathUtil.dateMonthAdd(loanObj.final_time, loanObj.day_month, -loanObj.deadline + period+1);
                                    console.log(moment(realFirstAccrueDay).format("YYYY-MM-DD"));
                                    firstMoneyDaysNum= dateMathUtil.daysBetween(realFirstAccrueDay, loanObj.accrue_time);
                                    console.log('==');
                                    console.log(firstMoneyDaysNum);
                                    //上一次线下还款日到第一次还款日时间差
                                    var outRepayTime = dateMathUtil.dateMonthAdd(loanObj.final_time, loanObj.day_month, -loanObj.deadline+1);
                                    console.log(moment(outRepayTime).format("YYYY-MM-DD"));
                                    firstMoneyOutDaysNum= dateMathUtil.daysBetween(realFirstAccrueDay, outRepayTime);
                                    console.log('==');
                                    console.log(firstMoneyOutDaysNum);
                                    ...
                                    loanRepayObjArr[_index].repay_day=dateMathUtil.dateAdd('d',loanObj.final_time, -1);
                                    loanRepayObjArr[_index].interest = 0;
                                }
                                else {
                                    var tempMonth=preRepayDay.getMonth()+2;
                                    loanRepayObjArr[_index].repay_day = dateMathUtil.pointDay(tempMonth,loanObj.rhday);
                                    loanRepayObjArr[_index].interest = sumInterest;
                                }
                                preRepayDay=loanRepayObjArr[_index].repay_day;
                                loanRepayObjArr[_index].loan_id = loanObj.id;
                                loanRepayObjArr[_index].st_interest = 0;//-----需叠加
                                //loanRepayObjArr[_index].create_time = dateUtil.getCurTime(); //保存时记录
                                loanRepayObjArr[_index].period = period;
