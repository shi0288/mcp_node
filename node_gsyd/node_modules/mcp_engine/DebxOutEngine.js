...
DebxOutEngine.prototype.ptx = function (loanObj, repayTypeObj, jt, cb) {
    ...
    cursor.count(function (err, count) {
        ...
        //正在处理的投资人顺序索引
        var order = 1;
        var loanRepayObjArr = calculateService.outDebxCal(loanObj);
                    ...
                    async.eachSeries(data, function (investObj, callback) {
                        ...
                        async.eachSeries(hadRepayObjArr, function (obj, repayCall) {
                            ...
                            if (order == count) {
                                ...
                                interest = Math.round(investObj.money / loanObj.money * loanRepayObjArr[_index].interest);
                                hadRepayObjArr[_index].hadInterest += interest;
                            }
                            //计算本金
                            var money = Math.round(investObj.money / loanObj.money * loanRepayObjArr[_index].money);
                            if (period == loanObj.deadline) {
                                money = investObj.money - tempMoney;
                            }else{
                                tempMoney += money;
                            }
                            //即投计算
